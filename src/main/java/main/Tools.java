...
public class Tools {
    ...
    public static WB_PolyLine toWB_PolyLine(LineString ls) {
        Coordinate[] pts = ls.getCoordinates();
        WB_Point[] polypt = new WB_Point[ls.getNumPoints()];

        for (int i = 0; i < pts.length; ++i) {
            polypt[i] = new WB_Point(pts[i].x, pts[i].y);
        }
        return new WB_GeometryFactory().createPolyLine(polypt);
    }

    public static WB_Segment toWB_Segment(LineString ls) {
        Coordinate[] pts = ls.getCoordinates();
        WB_Point[] polypt = new WB_Point[ls.getNumPoints()];
        ...
        return new WB_GeometryFactory().createSegment(polypt[0], polypt[pts.length-1]);
    }
...
//		WB_Circle c = new WB_Circle(pts.xd(), pts.yd(), r);
    ...
    public void drawPoint(final Coordinate pt, final double r, int[] co) {
        WB_Circle c = new WB_Circle(new WB_Point(pt.getX(), pt.getY(), 0.1), r);

        app.fill(co[0], co[1], co[2]);
        render.drawCircle(c);
    }

    public void drawPoint(final Coordinate pt, final double r) {
        WB_Circle c = new WB_Circle(new WB_Point(pt.getX(), pt.getY(), 0.05), r);
